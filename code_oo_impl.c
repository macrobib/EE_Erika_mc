 ...
 * Budget timer based on counter 0 RTI interrupt.
...
/*Watchdog timer based on Digital Watchdog Windowed(DWWD) timer.
 * */
inline void rearm_watchdog_timer(uint32 budget){
	if(EE_as_active_trans == EE_AS_TO_NONE){

		 dwdSetPreload(budget);
		 dwdReset();
}

/*DWWD callback timer to indicate a budget overrun.*/
void esmGroup2Notification(uint32 channel){
	/*Clear the status flag, and reset the window value to arbitrary high value
	 * while in the transition state in criticality.*/
	/*In case of an active task with budget more than the window size of the watchdog keep track of the chunk remaining.
	 * */
	dwdClearFlag();
	rearm_watchdog_timer(4095);
}
