...
extern void EE_std_change_context(EE_TID tid);
//#define EE_std_change_context  EE_cortex_rx_change_context
...
__INLINE__ EE_UREG __ALWAYS_INLINE__ EE_hal_swfrt_get_current_time ( void )
{
  ...
  return (rtiGetCurrentTick(1) & EE_HAL_SWFRT_TIMER_DURATION);
}
...
__INLINE__ EE_UREG __ALWAYS_INLINE__ EE_hal_tp_get_current_time_and_pause(void){
    return 0;
}

__INLINE__ EE_UREG __ALWAYS_INLINE__ EE_hal_tp_set_expiration(EE_UREG expiration){
    return 0;
}

__INLINE__ EE_UREG __ALWAYS_INLINE__ EE_hal_tp_stop(void){
    return 0;
}
