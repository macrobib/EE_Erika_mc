...
#include "ee_as_sched_crit_handler.h"
...
/**Timing protection free running counter support for ERIKA.**/
#ifdef EE_TIMING_PROTECTION_
static inline EE_UREG EE_hercules_get_curr_time(void){
	/*Read counter*/
	return val;
}

static inline EE_UREG EE_hercules_eval_elapsed_time(EE_UREG after, EE_UREG before){
	return after - before;
}

static inline EE_UREG EE_hercules_eval_get_elapsed_time(EE_UREG before){
	return EE_hercules_eval_elapsed_time(before, EE_hercules_get_curr_time());
}
#endif
